...
def get_model(num_dim_x, num_dim_control, w_lb, use_cuda = False):
    model_Wbot = torch.nn.Sequential(
        torch.nn.Linear(num_dim_x-num_dim_control, 128, bias=True),
        torch.nn.Tanh(),
    ...
    def W_func(x):
        ...
        Wbot = model_Wbot(x[:, 0:num_dim_x-num_dim_control]).view(bs, num_dim_x-num_dim_control, num_dim_x-num_dim_control)
